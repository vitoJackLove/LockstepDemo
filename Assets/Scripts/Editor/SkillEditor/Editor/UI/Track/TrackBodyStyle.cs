...
{
    public static void UpdateUI(SkillTimelineEditorWindow window, Rect rect, int[] highLight, float widthPreFrame,
        TrackStyleData data,
        int trackIndex)
    {
        //绘制背景
        ...
        for (int i = 0; i < data.Clips.Count; i++)
        {
            ...
            TrackClipStyle.UpdateUI(window, clipRect, highLight, data.Color, clipData.Name, trackIndex, i,
                widthPreFrame, data.UpdateUI, data.OverrideUI);
        }
    ...
    private static void ProcessEvent(SkillTimelineEditorWindow window, Rect rect, int[] highLight, float widthPreFrame,
