...
public static class TrackClipStyle
{
    ...
    public static void UpdateUI(SkillTimelineEditorWindow window, Rect rect, int[] highLight, Color color,
        string nam, int trackIndex,
        int clipIndex, float widthPerFrame, ClipUIAction additoin, bool overrideUI)
    {
        if (!overrideUI)
        {
            ...
            //绘制Cip颜色标识
            EditorGUI.DrawRect(new Rect(rect.x, rect.y + rect.height * 0.8f, rect.width, rect.height * 0.2f), color);

            //绘制名称
            GUI.contentColor = Color.white;
            GUI.Label(rect, nam, "Box");
            ...
            //额外的绘制
            additoin?.Invoke(window, rect, highLight, color, nam, widthPerFrame, trackIndex, clipIndex);
        }
        else
        {
            //额外的绘制
    ...
    private static void ProcessEvent(SkillTimelineEditorWindow window, Rect rect, int trackIndex, int clipIndex)
    {
        ...
        //Ctrl S
        if (UnityEngine.Event.current.type == UnityEngine.EventType.KeyDown && UnityEngine.Event.current.control &&
            UnityEngine.Event.current.keyCode == KeyCode.S)
        {
            ...
            keyboradEvent.Shortcut = Shortcut.CtrlS;
            EventCenter.TrigerEvent(window, keyboradEvent);
            UnityEngine.Event.current.Use();
        }
