...
namespace SuperScrollView
{
    ...
    public class LoopListView2 : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        public float MPaddingTop => mPaddingTop;
        [SerializeField]
        ...
        */
        ...
        void AdjustPanelPos()
        {
            ...
            else if (mArrangeType == ListItemArrangeType.LeftToRight_Center)
            {
                if (contentSize <= viewPortSize)
                {
                    ...
                    pos.x = 0;
                    mContainerTrans.anchoredPosition3D = pos;
                    float posX = 0 - mContainerTrans.rect.width / 2 + mItemList[0].CachedRectTransform.rect.width / 2;
                    mItemList[0].CachedRectTransform.anchoredPosition3D = new Vector3(posX, mItemList[0].StartPosOffset, 0);
                    UpdateAllShownItemsPos();
                    return;
                }
                LoopListViewItem2 tViewItem0 = mItemList[0];
        ...
        void UpdateSnapMove(bool immediate = true, bool forceSendEvent = false)
        {
        ...
        void UpdateSnapVertical(bool immediate = false, bool forceSendEvent = false)
        {
            ...
            if (needCheck)
            {
                ...
                if (curIndex >= 0)
                {
                    ...
                    if (forceSendEvent || oldNearestItemIndex >= 0 && mItemList[curIndex].ItemIndex != oldNearestItemIndex)
                    {
        ...
        bool UpdateForHorizontalList(float distanceForRecycle0, float distanceForRecycle1, float distanceForNew0, float distanceForNew1)
        {
            ...
            else if (mArrangeType == ListItemArrangeType.LeftToRight_Center)
            {
                if (mItemList.Count == 0)
                {
                    ...
                    mItemList.Add(newItem);
                    pos = pos - this.ContainerTrans.rect.width / +newItem.CachedRectTransform.rect.width / 2;
                    newItem.CachedRectTransform.anchoredPosition3D = new Vector3(pos, newItem.StartPosOffset, 0);
                    UpdateContentSize();
                    return true;
                }
                ...
                if (mViewPortRectLocalCorners[1].x - leftPos1.x < distanceForNew1)
                {
                    ...
                    if (nIndex <= mCurReadyMaxItemIndex || mNeedCheckNextMaxItem)
                    {
                        ...
                        else
                        {
                            ...
                            UpdateContentSize();
                            float newX = 0 - this.ContainerTrans.rect.width / 2 + mItemList[0].CachedRectTransform.rect.width / 2;
                            mItemList[0].CachedRectTransform.anchoredPosition3D = new Vector3(newX, newItem.StartPosOffset, 0);

                            for (int i = 1; i < mItemList.Count; i++)
                            {
                                newX = mItemList[i - 1].CachedRectTransform.anchoredPosition3D.x + mItemList[i - 1].CachedRectTransform.rect.width + mItemList[i - 1].Padding;
                                mItemList[i].CachedRectTransform.anchoredPosition3D = new Vector3(newX, newItem.StartPosOffset, 0);
                            }
        ...
        void UpdateItemArrangeType(RectTransform rtf)
        {
            AdjustAnchor(rtf);
            AdjustPivot(rtf);
        }
        ...
        void UpdateAllShownItemsPos()
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    UpdateItemArrangeType(item.CachedRectTransform);
            ...
            else if (mArrangeType == ListItemArrangeType.LeftToRight_Center)
            {
                ...
                float pos1 = mItemList[0].CachedRectTransform.anchoredPosition3D.x;
                float d = pos - pos1;
                float curX = pos - mContainerTrans.rect.width / 2 + mItemList[0].CachedRectTransform.rect.width / 2;
                for (int i = 0; i < count; ++i)
                {
                    LoopListViewItem2 item = mItemList[i];
                    UpdateItemArrangeType(item.CachedRectTransform);
        ...
        void UpdateContentSize()
        {
            ...
            if (ArrangeType == ListItemArrangeType.BottomToTop || ArrangeType == ListItemArrangeType.TopToBottom)
            {
                size += mPaddingTop + mPaddingBottom;
            }
            else if (ArrangeType == ListItemArrangeType.RightToLeft || ArrangeType == ListItemArrangeType.LeftToRight)
            {
                size += mPaddingLeft + mPaddingRight;
            }
