...
public class FrameSyncClientComponent : RunTimeComponent
{
    ...
    public void Send(IObserverParams content, BattleObserverEventEnum messageType)
    {
        ...
        try
        {
            INetworkPacketCodec codec = NetworkPacketCodecProvider.Instance;
            if (codec == null)
            {
                GameLog.Error(GameLogChannel.Network, "Network packet codec is not registered.");
                return;
            }

            byte[] packetData = codec.SerializePacket(messageType, content);
    ...
    private void ReceiveData()
    {
        while (_isRunning && _client != null && _client.Connected)
        {
            try
            {
                ...
                if (codec == null)
                {
                    GameLog.Warn(GameLogChannel.Network, "Network packet codec is not registered.");
                    continue;
                }

                if (!codec.TryDeserializePacket(packetData, out BattleObserverEventEnum messageType, out IObserverParams observerParams))
                {
