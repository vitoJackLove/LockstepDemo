...
public class CanvasComponent : RunTimeComponent
{
    ...
    public override void Init()
    {
        ...
        for (int i = 0; i < m_CanvasGroups.Length; i++)
        {
            ...
            string canvasGroupName = canvasGroup.Name;
            if (string.IsNullOrEmpty(canvasGroupName))
            {
                GameLog.Warn(GameLogChannel.UI, "Skip canvas group with empty name.");
                continue;
            }

            if (m_CacheCanvasGroups.ContainsKey(canvasGroupName))
            {
                GameLog.Warn(GameLogChannel.UI, $"Duplicate canvas group name skipped: {canvasGroupName}");
                continue;
            }

            m_CacheCanvasGroups.Add(canvasGroupName, canvasGroup);
        }
    }
