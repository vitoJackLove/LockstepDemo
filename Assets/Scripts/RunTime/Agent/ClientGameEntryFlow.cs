...
public static class ClientGameEntryFlow
{
        ...
        GameSessionModeType sessionMode = GameSessionModeType.Online)
    {
        ...
        {
            GameLog.Error(GameLogChannel.AgentTest, "Create world failed. Team list is empty.");
            return false;
        }
        ...
        for (int i = 0; i < teamList.Count; i++)
        {
            ...
            GameLog.Info(GameLogChannel.AgentTest, $"Create world player[{i}]: heroId={playerData.HeroId}, serverEntityId={playerData.ServerEntityId}, isSelf={playerData.IsSelf}");
        }

        if (!hasSelfPlayer)
        {
            GameLog.Error(GameLogChannel.AgentTest, $"Create world failed. Team list has no self player. playerCount={teamList.Count}");
            return false;
        }
