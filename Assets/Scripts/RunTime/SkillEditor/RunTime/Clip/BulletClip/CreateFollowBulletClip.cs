...
[ClipName("创建跟随子弹")]
public class CreateFollowBulletClip : TaskClip
{
    ...
    public override void EditorEnter(GameObject context, int fps, int currentFrameID)
    {
        ...
        fp3 position = TsUtil.TransformPoint(fpmath1.Vector3ToFp3(context.transform.position),
            fpmath1.Vector3ToFp3(context.transform.eulerAngles),
            new fp3(1, 1, 1), fpmath1.Vector3ToFp3(offset));
    ...
    {
        base.EditorTick(currentFrameID, fps, deltaTime, context);

        if (context)
        {
            fp3 position = TsUtil.TransformPoint(fpmath1.Vector3ToFp3(context.transform.position), fpmath1.Vector3ToFp3(context.transform.eulerAngles), 
                new fp3(1,1,1), fpmath1.Vector3ToFp3(offset));

            _bulletGo.transform.position = fpmath1.Fp3ToVector3(position);
        }
    }
    ...
    public override void OnRunTimeEnter(BaseEntity context, int fps)
    {
        ...
        fp3 position = TsUtil.TransformPoint(context.transform.Position, context.transform.EulerAngles, fp3.zero, fpmath1.Vector3ToFp3(offset));
